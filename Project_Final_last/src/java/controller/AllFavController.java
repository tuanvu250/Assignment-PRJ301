 ...
 * @author ADMIN
 */
@WebServlet(name = "AllFavController", urlPatterns = {"/AllFavController"})
public class AllFavController extends HttpServlet {

    private static final String PRODUCT_PAGE = "/products/product.jsp";
    private ShoesProductDAO shoesDAO = new ShoesProductDAO();
    
    protected String processReadAllFav(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (action != null) {
                if (action.equals("readAll")) {
                    url = processReadAllFav(request, response);
